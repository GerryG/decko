...
decko_namespace = namespace :decko do
  ...
  task :reset_tmp do
    tmp_dir = Decko.application.config.paths["tmp"].first
    if Decko.application.config.paths["tmp"].existent
  ...
  def prepped_asset_path
    return if Rails.root.to_s == Decko::DECKO_GEM_ROOT # inside decko gem
